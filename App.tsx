...
const BottomNav = ({ active, onPress }: { active: TabKey; onPress: (key: TabKey) => void }) => {
  ...
  const bubbleX = slideAnim.interpolate({
    ...
    outputRange: pillWidth > 0
      ? [0, 1, 2, 3].map(i => tabW * i + tabW / 2 - BUBBLE / 2)
      : [0, 0, 0, 0],
  });
        ...
        {items.map(({ image, label, key }) => {
          ...
          return (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
              onPress={() => { haptic.light(); onPress(key); }}
            >
              {/* Icon slot — hidden when active (bubble shows it instead) */}
              <View style={{ height: 22, justifyContent: 'center', alignItems: 'center' }}>
                {!isActive && (
                  <Image source={image} style={{ width: 20, height: 20, opacity: 0.4 }} resizeMode="contain" />
                )}
              </View>
              <Text className={JUA} style={{
