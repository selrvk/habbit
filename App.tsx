...
const NumpadModal = ({
  visible, title, hint, confirmLabel, amount, currency,
  onChangeAmount, onConfirm, onClose,
}: {
  visible: boolean; title: string; hint?: string; confirmLabel: string;
...
// ─── Text Input Modal (name edit) ─────────────────────────────────────────────

const TextModal = ({
  visible, title, placeholder, initialValue, onSave, onClose,
}: {
              ...
              <Animated.View style={{ transform: [{ scale: scaleAnim }], width: '86%', backgroundColor: '#3B2220', borderRadius: 24, padding: 22, borderWidth: 1.5, borderColor: 'rgba(212,149,106,0.35)', shadowColor: '#000', shadowOffset: { width: 0, height: 14 }, shadowOpacity: 0.55, shadowRadius: 28, elevation: 24 }}>
                ...
                <View style={{ backgroundColor: '#5C3D2E', borderRadius: 14, borderWidth: 1.5, borderColor: canSave ? '#D4956A' : 'rgba(212,149,106,0.2)', paddingHorizontal: 16, paddingVertical: 4, marginBottom: 20 }}>
                  <TextInput
                    ...
                    placeholder={placeholder} placeholderTextColor="rgba(232,213,192,0.25)"
                    returnKeyType="done" onSubmitEditing={() => canSave && onSave(text.trim())} maxLength={24}
        ...
        <TouchableWithoutFeedback onPress={onClose}>
          <Animated.View style={{ flex: 1, backgroundColor: 'rgba(18,7,5,0.80)', justifyContent: 'center', alignItems: 'center', opacity: opacityAnim }}>
            <TouchableWithoutFeedback onPress={() => {}}>
              <Animated.View style={{ transform: [{ scale: scaleAnim }], width: '86%', backgroundColor: '#3B2220', borderRadius: 24, padding: 22, borderWidth: 1.5, borderColor: 'rgba(212,149,106,0.35)', shadowColor: '#000', shadowOffset: { width: 0, height: 14 }, shadowOpacity: 0.55, shadowRadius: 28, elevation: 24 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                  <Text className={`${JUA} text-accent text-xs tracking-widest uppercase`} style={{ opacity: 0.7 }}>{isEdit ? 'Edit Commission' : 'New Commission'}</Text>
                  <TouchableOpacity onPress={onClose} activeOpacity={0.7} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}>
                    <Text style={{ color: 'rgba(232,213,192,0.4)', fontSize: 16 }}>✕</Text>
                  </TouchableOpacity>
                </View>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  <TouchableOpacity onPress={onClose} activeOpacity={0.7} style={{ flex: 1, backgroundColor: 'rgba(212,149,10,0.1)', borderWidth: 1, borderColor: 'rgba(212,149,106,0.25)', borderRadius: 14, paddingVertical: 14, alignItems: 'center' }}>
                    <Text className={`${JUA} text-sm`} style={{ color: 'rgba(232,213,192,0.55)' }}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={canSave ? handleSave : undefined} activeOpacity={canSave ? 0.8 : 1} style={{ flex: 2, backgroundColor: canSave ? '#D4956A' : 'rgba(212,149,106,0.2)', borderRadius: 14, paddingVertical: 14, alignItems: 'center', shadowColor: canSave ? '#D4956A' : 'transparent', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: canSave ? 6 : 0 }}>
                    <Text className={DYNAPUFF} style={{ color: canSave ? '#fff' : 'rgba(255,255,255,0.3)', fontSize: 15 }}>{isEdit ? 'Save Changes' : 'Add Commission'}</Text>
                  </TouchableOpacity>
                </View>
              </Animated.View>
            </TouchableWithoutFeedback>
...
const BottomNav = ({ active, onPress }: { active: TabKey; onPress: (key: TabKey) => void }) => {
  const items: { icon: string; label: string; key: TabKey }[] = [
    { icon: '🏠', label: 'Home', key: 'home' }, { icon: '📋', label: 'Habbits', key: 'tasks' },
    { icon: '🥕', label: 'Finance', key: 'finance' }, { icon: '🐰', label: 'Profile', key: 'profile' },
  ];
  return (
...
const ProfileScreen = ({
  ...
  const handleReset = () => {
    Alert.alert(
      "Reset Today's Data",
      "This will uncheck all of today's commissions and clear today's spending. Are you sure?",
      [
  ...
  const StatCard = ({ emoji, label, value, accent = false }: { emoji: string; label: string; value: string; accent?: boolean }) => (
    <View style={{
      flex: 1, backgroundColor: '#5C3D2E', borderRadius: 16, padding: 16, alignItems: 'center',
      borderWidth: 1, borderColor: accent ? 'rgba(212,149,106,0.4)' : 'rgba(212,149,106,0.15)',
  ...
  return (
    <>
      <TextModal
        visible={nameModal} title="Edit Name" placeholder="Your name"
        initialValue={name} onSave={(v) => { onSetName(v); setNameModal(false); }}
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 28, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ alignItems: 'center', marginBottom: 8 }}>
          {/* Avatar circle */}
          <View style={{
            width: 90, height: 90, borderRadius: 45, backgroundColor: '#5C3D2E',
            justifyContent: 'center', alignItems: 'center', marginBottom: 14,
            borderWidth: 2.5, borderColor: '#D4956A',
            shadowColor: '#D4956A', shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.45, shadowRadius: 12, elevation: 8,
          }}>
          ...
          {AVATARS.map(a => (
            <TouchableOpacity
              ...
              style={{
                ...
                backgroundColor: avatar === a ? 'rgba(212,149,106,0.2)' : '#5C3D2E',
                justifyContent: 'center', alignItems: 'center',
                borderWidth: 2, borderColor: avatar === a ? '#D4956A' : 'rgba(212,149,106,0.15)',
                shadowColor: avatar === a ? '#D4956A' : 'transparent',
                shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 8, elevation: avatar === a ? 4 : 0,
              }}
            >
        ...
        {/* Total completed full-width */}
        <View style={{
          backgroundColor: '#5C3D2E', borderRadius: 16, padding: 16,
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <View>
              <Text className={JUA} style={{ fontSize: 11, color: 'rgba(232,213,192,0.5)', marginBottom: 2 }}>Total Commissions Completed</Text>
        ...
        {stats.currentStreak === 0 && (
          <Text className={JUA} style={{ fontSize: 11, color: 'rgba(232,213,192,0.3)', textAlign: 'center', marginTop: 6, marginBottom: 4 }}>
            Complete all commissions today to start your streak! 🔥
          </Text>
        )}
        ...
        <TouchableOpacity
          ...
          style={{
            backgroundColor: 'rgba(200,80,60,0.1)', borderRadius: 16, paddingVertical: 16,
            alignItems: 'center', borderWidth: 1, borderColor: 'rgba(200,80,60,0.3)',
          }}
        >
          <Text className={DYNAPUFF} style={{ color: '#f09090', fontSize: 15 }}>Reset Today's Data</Text>
          <Text className={JUA} style={{ color: 'rgba(240,144,144,0.5)', fontSize: 11, marginTop: 3 }}>
            Unchecks Habbits · clears spending
          </Text>
        </TouchableOpacity>
...
const FinanceScreen = ({
  ...
  onSetAllocated, onSetCurrency, onUndoEntry,
}: {
  ...
  const handleUndoConfirm = (entry: SpendingEntry) => {
    Alert.alert('Undo Entry', `Remove ${currency}${entry.amount.toFixed(2)} added at ${entry.time}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Undo', style: 'destructive', onPress: () => onUndoEntry(entry.id) },
    ]);
  };
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 20, paddingBottom: 32 }} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity onPress={() => setBudgetModal(true)} activeOpacity={0.8} className="bg-card rounded-2xl p-4 mb-3 flex-row items-center justify-between" style={{ borderWidth: 1, borderColor: 'rgba(212,149,106,0.25)', shadowColor: '#1a0a08', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.15, shadowRadius: 4, elevation: 3 }}>
          <View>
            ...
            <View className="flex-row items-center gap-x-1.5"><Text className="text-base">🥕</Text><Text className={`${DYNAPUFF} text-cream text-xl`}>{currency}{allocatedPerDay.toFixed(2)}</Text></View>
          </View>
          <View style={{ backgroundColor: 'rgba(212,149,106,0.12)', borderRadius: 10, paddingVertical: 6, paddingHorizontal: 14, borderWidth: 1, borderColor: 'rgba(212,149,106,0.3)' }}>
            <Text className={`${JUA} text-xs`} style={{ color: '#D4956A' }}>Edit</Text>
          </View>
        </TouchableOpacity>
...
const TasksScreen = ({ commissions, onAdd, onEdit, onDelete }: { commissions: Commission[]; onAdd: (l: string) => void; onEdit: (id: string, l: string) => void; onDelete: (id: string) => void }) => {
  ...
  const handleDelete = (item: Commission) => Alert.alert('Remove Commission', `Remove "${item.label}"?`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Remove', style: 'destructive', onPress: () => onDelete(item.id) }]);
...
const HomeScreen = ({
  commissions, setCommissions, spentToday, setSpentToday,
  todayHistory, setTodayHistory, allocatedPerDay, currency,
  name, avatar, onGoToTasks, onCommissionComplete, onCommissionUncomplete,
}: {
  commissions: Commission[];
  setCommissions: React.Dispatch<React.SetStateAction<Commission[]>>;
  spentToday: number;
  setSpentToday: React.Dispatch<React.SetStateAction<number>>;
  todayHistory: SpendingEntry[];
  setTodayHistory: React.Dispatch<React.SetStateAction<SpendingEntry[]>>;
  allocatedPerDay: number;
  currency: string;
  name: string;
  avatar: string;
  onGoToTasks: () => void;
  onCommissionComplete: (id: string) => void;
  onCommissionUncomplete: (id: string) => void;
}) => {
  ...
  return (
    <>
      ...
      <ScrollView className="flex" contentContainerClassName="px-4 pt-3.5 pb-6" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        {/* Header — uses name and avatar from profile */}
        <View className="flex-row items-center gap-x-3.5 mb-6">
          ...
          <View>
            ...
            <Text className={`${DYNAPUFF} text-cream text-2xl tracking-wide`}>{name}</Text>
          </View>
        </View>
        ...
        <View className="bg-card rounded-2xl py-4 px-3.5 flex-row items-center mb-5" style={{ shadowColor: '#1a0a08', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.2, shadowRadius: 6, elevation: 4 }}>
          ...
          <View className="flex-1 items-center rounded-xl py-2" style={{ backgroundColor: allDone && hasCommissions ? 'rgba(100,160,90,0.12)' : 'rgba(212,149,106,0.08)' }}>
            ...
            <Text className={`${DYNAPUFF} text-sm mt-0.5`} style={{ color: allDone && hasCommissions ? '#9de087' : '#D4956A' }}>
              {!hasCommissions ? 'Not set' : allDone ? 'Nice one!' : `${completedCount}/${totalCount}`}
            </Text>
...
export default function App() {
      ...
      try {
        ...
        if (storedC) {
          ...
          if (parsed.date === todayKey) {
            ...
            // Archive completed count before reset
            const doneCount = parsed.items.filter(c => c.completed).length;
            if (doneCount > 0) {
              loadedStats = { ...loadedStats, totalCompleted: loadedStats.totalCompleted + doneCount };
            }
            // Check streak continuity
            const allWereDone = parsed.items.length > 0 && parsed.items.every(c => c.completed);
            if (allWereDone) {
              if (loadedStats.lastFullDate === yesterdayKey) {
                // streak already counted, just carries over
              } else if (parsed.date === yesterdayKey) {
                // yesterday was a full-complete day, increment streak
                const newStreak = loadedStats.currentStreak + 1;
                const newBest   = Math.max(newStreak, loadedStats.bestStreak);
                loadedStats = { ...loadedStats, currentStreak: newStreak, bestStreak: newBest, lastFullDate: parsed.date };
              }
            } else if (parsed.date < yesterdayKey) {
        ...
        // Check if streak broke overnight (no commissions yesterday, streak was active)
  ...
  // ── Check if all commissions just became done → update streak ─────────────

  useEffect(() => {
    if (commissions.length === 0) return;
    const allDone = commissions.every(c => c.completed);
    if (!allDone) return;
    if (stats.lastFullDate === todayKey) return; // already counted today
  ...
  const handleCommissionComplete = useCallback((id: string) => {
    ...
    setStats(prev => {
      const updated = { ...prev, totalCompleted: prev.totalCompleted + 1 };
      saveStats(updated);
      return updated;
    });
  }, []);
  ...
  const handleCommissionUncomplete = useCallback((id: string) => {
    ...
    setStats(prev => {
      const updated = { ...prev, totalCompleted: Math.max(prev.totalCompleted - 1, 0) };
      saveStats(updated);
      return updated;
    });
  }, []);
  ...
  const handleResetToday = useCallback(() => {
    ...
    // Reset streak for today if it was set today
    setStats(prev => {
      if (prev.lastFullDate !== todayKey) return prev;
      const newStreak = prev.currentStreak > 0 ? prev.currentStreak - 1 : 0;
      const updated = { ...prev, currentStreak: newStreak, lastFullDate: yesterdayKey };
      saveStats(updated);
      return updated;
    });
  ...
  const renderScreen = () => {
    switch (activeTab) {
      case 'home':
        return (
          <HomeScreen
            ...
            spentToday={spentToday} setSpentToday={setSpentToday}
            todayHistory={todayHistory} setTodayHistory={setTodayHistory}
            allocatedPerDay={allocatedPerDay} currency={currency}
      ...
      case 'finance':
        return (
          <FinanceScreen
            spentToday={spentToday} todayHistory={todayHistory}
            dailyTotals={dailyTotals} allocatedPerDay={allocatedPerDay} currency={currency}
            onSetAllocated={handleSetAllocated} onSetCurrency={handleSetCurrency}
