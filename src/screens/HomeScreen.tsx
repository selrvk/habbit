  ...
  return (
    <>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 20, paddingBottom: navHeight }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" scrollEnabled={scrollEnabled}>
        ...
        {/* Today's Performance */}
        <Text style={{ fontFamily: 'Jua', color: '#D4956A', fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>Today's Performance</Text>
        <View style={{ backgroundColor: '#5C3D2E', borderRadius: 16, paddingVertical: 16, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', marginBottom: 20, shadowColor: '#1a0a08', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.2, shadowRadius: 6, elevation: 4 }}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.7 }}>{today.dayName}</Text>
            <Text style={{ fontFamily: 'DynaPuff', color: '#e8d5c0', fontSize: 16, lineHeight: 20 }}>{today.month} {today.date}</Text>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.7 }}>{today.year}</Text>
          </View>
          ...
          <View style={{ flex: 1, alignItems: 'center', borderRadius: 12, paddingVertical: 8, backgroundColor: commissionsGood ? 'rgba(100,160,90,0.12)' : 'rgba(212,149,106,0.08)' }}>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.75 }}>Habbits</Text>
            <Text style={{ fontFamily: 'DynaPuff', fontSize: 14, marginTop: 2, color: commissionsGood ? '#9de087' : '#D4956A' }}>
          ...
          <View style={{ flex: 1, alignItems: 'center', borderRadius: 12, paddingVertical: 8, backgroundColor: financeGood ? 'rgba(100,160,90,0.12)' : 'rgba(200,80,60,0.12)' }}>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.75 }}>Finance</Text>
            <Text style={{ fontFamily: 'DynaPuff', fontSize: 14, marginTop: 2, color: financeGood ? '#9de087' : '#f09090' }}>{financeGood ? 'On track!' : 'Oh no...'}</Text>
        ...
        {!hasCommissions ? (
          <View style={{ alignItems: 'center', paddingVertical: 32, paddingHorizontal: 16 }}>
            ...
            <Text style={{ fontFamily: 'DynaPuff', color: '#e8d5c0', fontSize: 16, marginBottom: 8 }}>No Habbits set!</Text>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.5, textAlign: 'center', marginBottom: 20 }}>Head over to the Habbits tab to{'\n'}add your daily Habbits.</Text>
            <TouchableOpacity onPress={() => { haptic.light(); onGoToTasks(); }} activeOpacity={0.8}
              ...
              <Text style={{ fontFamily: 'Jua', color: '#D4956A', fontSize: 14 }}>Go to Habbits →</Text>
            </TouchableOpacity>
          </View>
        ) : !hasTodayCommissions ? (
          <View style={{ alignItems: 'center', paddingVertical: 32, paddingHorizontal: 16 }}>
            <Text style={{ fontSize: 44, marginBottom: 12 }}>😴</Text>
            <Text style={{ fontFamily: 'DynaPuff', color: '#e8d5c0', fontSize: 16, marginBottom: 8 }}>Rest day!</Text>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.5, textAlign: 'center' }}>No Habbits scheduled for {today.dayName}.{'\n'}Enjoy your break! 🐰</Text>
          </View>
        ) : (
          <>
            <View style={{ alignItems: 'center', marginBottom: 4 }}>
              <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 6 }}>
                <Text style={{ fontFamily: 'DynaPuff', fontSize: 36, color: allDone ? '#9de087' : '#D4956A' }}>{completedCount}</Text>
                <Text style={{ fontFamily: 'DynaPuff', color: '#e8d5c0', fontSize: 20, opacity: 0.6, marginBottom: 2 }}>/{totalCount}</Text>
                <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 16, opacity: 0.7, marginBottom: 2, marginLeft: 4 }}>finished</Text>
              </View>
            ...
            <Text style={{ fontFamily: 'Jua', color: '#D4956A', fontSize: 14, textAlign: 'center', marginBottom: 16, opacity: 0.7 }}>{allDone ? '🎉 All done for today!' : 'Swipe right to complete · left to undo'}</Text>
            ...
            {completedTasks.length > 0 && (
              <>
                <TouchableOpacity style={{ alignItems: 'center', marginTop: 4, marginBottom: 4, paddingVertical: 8 }} onPress={() => { haptic.light(); setShowCompleted(v => !v); }}>
                  <Text style={{ fontFamily: 'Jua', color: '#D4956A', fontSize: 14, opacity: 0.8 }}>Completed ({completedTasks.length}) {showCompleted ? '∧' : '›'}</Text>
                </TouchableOpacity>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          <View style={{ flex: 1, backgroundColor: '#5C3D2E', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: isOverBudget ? 'rgba(200,80,60,0.4)' : 'rgba(212,149,106,0.2)' }}>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.7, marginBottom: 8 }}>Spent Today</Text>
          ...
          <View style={{ flex: 1, backgroundColor: '#5C3D2E', borderRadius: 16, padding: 14, borderWidth: 1, borderColor: 'rgba(212,149,106,0.2)' }}>
            <Text style={{ fontFamily: 'Jua', color: '#e8d5c0', fontSize: 12, opacity: 0.7, marginBottom: 8 }}>Allocated a Day</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <Image source={IMAGES.carrots} style={{ width: 22, height: 22 }} resizeMode="contain" />
              <Text style={{ fontFamily: 'DynaPuff', color: '#e8d5c0', fontSize: 18 }}>{currency}{allocatedPerDay.toFixed(2)}</Text>
            </View>
          </View>
        </View>
        ...
        <TouchableOpacity style={{ backgroundColor: '#D4956A', borderRadius: 16, paddingVertical: 16, alignItems: 'center', shadowColor: '#D4956A', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 10, elevation: 6 }}
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontFamily: 'DynaPuff', color: '#fff', fontSize: 16 }}>Add to Spent Today</Text>
          </View>
        </TouchableOpacity>
      </ScrollView>
    </>
